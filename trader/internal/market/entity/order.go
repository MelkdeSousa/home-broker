...
type Order struct {
	...
	OrderType     OrderType      // Tipo da ordem: "BUY" (compra) ou "SELL" (venda)
	Status        OrderStatus    // Status da ordem: "OPEN" (aberta) ou "CLOSED" (fechada)
...
func NewOrder(orderID string, investor *Investor, asset *Asset, shares int, price float64, orderType OrderType) *Order {
	...
	return &Order{
		...
		Price:         price,
		OrderType:     orderType,
		Status:        OrderOpen, // Ordem começa com status aberto
